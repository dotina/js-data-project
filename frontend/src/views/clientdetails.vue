...
<template>
  <main>
    ...
    <div class="px-10 py-20">
      ...
      <!-- grid container -->
      <div class="mt-10 mb-12 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-x-6 gap-y-10">
        <h2 class="text-2xl font-bold">Profile picture</h2>

        <!-- Add file input to update profile picture if the user role is editor and hide it if the user role is viewer -->
        <div v-if="user.role === 'editor'" class="flex flex-col">
          <label>Change or update Profile picture</label>
          <label class="block">
            <input type="file" ref="imageFile" @change="handleFileInputChange" />
          </label>
        </div>

        <!-- Display preview of the profile picture -->
        <div v-if="selectedPicture">
          <img :src="selectedPicture" alt="Preview" class="mt-4 max-w-full h-auto" />
        </div>
        <div></div>
      ...
      <div class="flex flex-col col-span-2">
        <table class="min-w-full shadow-md rounded">
          ...
          <tbody class="divide-y divide-gray-300">
            <tr @click="$router.push({ name: 'eventdetails', params: { id: event._id } })" v-for="event in clientEvents"
              :key="event._id" class="cursor-pointer" :class="{ 'hoverRow': hoverId === event._id }"
              @mouseenter="hoverId = event._id" @mouseleave="hoverId = null">
    ...
    <div v-if="isLoading" class="loader-container">
      ...
      <confirmation-dialog :message="confirmationMessage" @confirm="handleConfirm"
        @cancel="handleCancel" ref="confirmationDialog"></confirmation-dialog>
...
import ConfirmationDialog from "@/components/ConfirmationDialog.vue"; // Import the confirmation dialog component
...
export default {
  ...
  components: {
    VueMultiselect,
    ConfirmationDialog, //add the confirmation dialog to the list of components
  },
  setup() {
  ...
  data() {
    return {
      ...
      hoverId: null,
      selectedPicture: null, // Add a property to store the selected picture
      confirmationMessage: "Are you sure you want to delete client?", // initialize the dialog message
      isLoading: false, // define isLoading variable to store loading state. Set initially to false
    }
  },
  validations() {
    // validations
  ...
  async mounted() {
    ...
    try {
      ...
      this.selectedPicture = this.client.imagePath==null? 'https://cdn-icons-png.flaticon.com/512/3541/3541871.png' : 'http://localhost:3000/uploads/'+this.client.imagePath;
      // this.selectedPicture = 

    } catch (error) {
      toast.error('error loading data:', error)
    } finally {
      // Set loading state to false after API calls are completed
  ...
  methods: {
    ...
    handleFileInputChange(event) {
      ...
      if (file) {
        this.client.files = this.$refs.imageFile.files[0];
        // Use FileReader to read the file and set it as the selected picture
    ...
    // method to open confirmation dialogue
    openConfirmationDialog() {
      this.$refs.confirmationDialog.show();
    },

    // method called when the dialog is confirmed
    handleConfirm() {
